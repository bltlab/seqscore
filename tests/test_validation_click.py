...
def test_invalid_BIO() -> None:
    ...
    assert (
        "Invalid transition O -> I-ORG for token 'University' on line 7" in result.output
    )
    assert "Invalid transition O -> I-LOC for token 'West' on line 12" in result.output
    assert (
...
def test_invalid_BIOES() -> None:
    ...
    assert (
        "Invalid transition I-ORG -> O after token 'Maryland' on line 20" in result.output
    )
    assert (
        "Invalid transition O -> I-ORG for token 'Department' on line 22" in result.output
    )
    assert (
        "Invalid transition O -> I-ORG for token 'University' on line 26" in result.output
    )
    assert (
        "Invalid transition I-ORG -> O after token 'Maryland' on line 28" in result.output
    )
    assert (
